...
function TopBar() {
  return (
    <div className="shadow-inner shadow-zinc-800">
        ...
        <div className="right-top flex flex-row gap-4">
          {info.facebook && (
            <Link href={info.facebook} target="_blank">
              <Facebook className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-zinc-800 dark:text-white" />
            </Link>
          )}
          {info.instagram && (
            <Link href={info.instagram} target="_blank">
              <Instagram className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-zinc-800 dark:text-white" />
            </Link>
          )}
          {info.linkedin && (
            <Link href={info.linkedin} target="_blank">
              <Linkedin className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-zinc-800 dark:text-white" />
            </Link>
          )}
          {info.twitter && (
            <Link href={info.twitter} target="_blank">
              <Twitter className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-zinc-800 dark:text-white" />
            </Link>
          )}
          {info.viber && (
            <Link href={info.viber} target="_blank">
              <MessageSquare className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-zinc-800 dark:text-white" />
            </Link>
          )}
          {info.whatsapp && (
            <Link href={info.whatsapp} target="_blank">
              <MessageCircle className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-zinc-800 dark:text-white" />
            </Link>
          )}
        </div>
      </div>
